...
// Color Pallete 
#define C_SKY_TOP       (Color){ 80, 140, 205, 255}
#define C_SKY_BOT       (Color){ 50,  90, 155, 255}
#define C_CLOUD         (Color){220, 230, 245, 200}
#define C_STAND_DARK    (Color){ 18,  10,  10, 255}
#define C_STAND_MID     (Color){ 28,  16,  16, 255}
#define C_CONCRETE      (Color){ 90,  82,  78, 255}
#define C_SEAT_RED      (Color){185,  28,  32, 255}
#define C_SEAT_DARK     (Color){140,  18,  22, 255}
#define C_SEAT_WHITE    (Color){230, 228, 222, 255}
#define C_GRASS_D       (Color){ 38, 100,  28, 255}
#define C_GRASS_L       (Color){ 52, 128,  36, 255}
#define C_GOLD          (Color){255, 210,   0, 255}
#define C_WHITE         (Color){255, 255, 255, 255}
#define C_BLACK         (Color){  0,   0,   0, 255}
#define C_FLOODLIGHT    (Color){255, 248, 200, 255}
#define C_PANEL_BG      (Color){  8,   4,   4, 210}
...
static void FillRect(int x, int y, int w, int h, Color c) {
    for (int row = y; row < y+h; row++) HLine(x, x+w-1, row, c);
}
...
static void DrawPitch(void) {
    ...
    for (int x = midX - arcWidth/2; x <= midX + arcWidth/2; x++) {
        ...
        DrawPixel(x, y, C_WHITE); DrawPixel(x, y+1, C_WHITE);
    }
}
...
static void DrawMenuPanel(const Menu *m) {
    ...
    DrawText(title, midX - tw/2 + 1, cy + 41, 26, (Color){0,0,0,160});
    DrawText(title, midX - tw/2,     cy + 40, 26, C_GOLD);

    const char *sub = "Arsenal  vs  Manchester United";
    int sw = MeasureText(sub, 15);
    DrawText(sub, midX - sw/2, cy + 72, 15, (Color){200,200,200,255});

    // Tombol Interaktif 
    int btn_w = (int)(panelW * 0.7f), btn_h = (int)(panelH * 0.15f);
    int btn_x = midX - btn_w/2;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        int fs = 18;
        DrawText(labels[i], midX - MeasureText(labels[i], fs)/2, btn_ys[i] + btn_h/2 - fs/2, fs, tcol);
    }

    const char *hint = "[UP/DOWN] Pilih   [ENTER] Konfirmasi   [ESC] Keluar";
    DrawText(hint, midX - MeasureText(hint,12)/2, py + panelH - 24, 12, (Color){120,120,120,255});
...
void Menu_Update(Menu *m) {
    ...
    /* Kontrol Keyboard */
    if (IsKeyPressed(KEY_UP)   || IsKeyPressed(KEY_W)) m->selected = (m->selected + 1) % 2;
    if (IsKeyPressed(KEY_DOWN) || IsKeyPressed(KEY_S)) m->selected = (m->selected + 1) % 2;
    if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_KP_ENTER)) m->chosen = (MenuChoice)m->selected;
    if (IsKeyPressed(KEY_ESCAPE)) m->chosen = MENU_EXIT;
    if (IsKeyPressed(KEY_ONE))  m->chosen = MENU_PLAY;
    if (IsKeyPressed(KEY_TWO))  m->chosen = MENU_EXIT;
    ...
    int panelW = (int)(SCREEN_W * 0.45f), panelH = (int)(SCREEN_H * 0.45f);
    int midX = SCREEN_W / 2, py = SCREEN_H / 2 - panelH / 2;
    int btn_w = (int)(panelW * 0.7f), btn_h = (int)(panelH * 0.15f);
    int btn_x = midX - btn_w/2;
    ...
    for (int i = 0; i < 2; i++) {
        if (mx >= btn_x && mx <= btn_x+btn_w && my >= btn_ys[i] && my <= btn_ys[i]+btn_h) {
            m->selected = i;
            if (IsMouseButtonPressed(0)) m->chosen = (MenuChoice)i;
        }
    }
}
...
void Menu_Draw(const Menu *m) {
    ...
    DrawTextureRec(
        ...
        (float)-menuCache.texture.height }, (Vector2){ 0, 0 }, WHITE);                  
