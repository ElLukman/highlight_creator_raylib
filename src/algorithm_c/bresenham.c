...
void Bres_DashedLine(int x1, int y1, int x2, int y2, int dashLen, int gapLen, Color color) {
    ...
    int sy = (y1 < y2) ? 1 : -1;
    int err = dx - dy;
    int counter = 0, drawing = 1, current_limit = dashLen;

    while (1) {
        if (drawing) DrawPixel(x1, y1, color);
        if (x1 == x2 && y1 == y2) break;
        int e2 = 2*err;
        if (e2 > -dy) { err -= dy; x1 += sx; }
        if (e2 <  dx) { err += dx; y1 += sy; }
        ...
        if (counter >= current_limit) {
            counter = 0; drawing = !drawing; current_limit = drawing ? dashLen : gapLen;
        }
    }
}
...
void Bres_DashDotLine(int x1, int y1, int x2, int y2, Color color) {
    ...
    while (1) {
        if (drawPh[phase % 4]) DrawPixel(x1, y1, color);
        if (x1 == x2 && y1 == y2) break;
        int e2 = 2*err;
        if (e2 > -dy) { err -= dy; x1 += sx; }
        if (e2 <  dx) { err += dx; y1 += sy; }
        counter++;
        if (counter >= phases[phase % 4]) { counter = 0; phase++; }
    }
}